...
def _merge_cookie_db(
    ...
    try:
        with contextlib.closing(
            sqlite3.connect(
                f"file:{pathname2url(dst_file)}",
                uri=True,
                timeout=_profile_remaining(deadline, 0.25),
            )
        ) as conn:
...
def _refresh_auth_file(
    ...
    if os.path.basename(src_file) == "Cookies" and os.path.isfile(dst_file):
        try:
            return _merge_cookie_db(src_file, dst_file, deadline=deadline)
        except _CookieMergeUnsupported as e:
            logger.info("real-profile: overwriting %s instead of merging (%s)", dst_file, e)
    return _copy_auth_file(src_file, dst_file, deadline=deadline)
...
def _mirror_profile_auth(
    ...
    for rel in _AUTH_REFRESH_PROFILE_FILES:
        ...
        if os.path.isfile(s) and not _refresh_auth_file(
